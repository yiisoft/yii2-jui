...
test:			## Run tests. Params: {{ v=8.1 }}.
	...
	docker exec yii2-jui-php-1 sh -c "php -v && composer update && vendor/bin/phpunit --coverage-clover=coverage.xml"
	make down
...
sh:			## Enter the container with the application
	docker exec -it yii2-jui-php-1 bash

static-analysis:	## Run code static analyze. Params: {{ v=8.1 }}.
	make build v=$(filter-out $@,$(v))
	docker exec yii2-jui-php-1 sh -c "php -v && composer update && vendor/bin/phpstan analyse --memory-limit 512M"
	make down
